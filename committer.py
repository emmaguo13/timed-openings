...
class Committer: 
    def __init__(self, M, bits=16): 
        ...
        while self.p2 % 4 != 3:
            self.p2 = getPrime(bits, randfunc=Crypto.Random.get_random_bytes)
        # p1 = random.randrange(3, 2**(bits) - 1, 4)
        # p2 = random.randrange(3, 2**(bits) - 1, 4)
    ...
    def compute_ys(self, challenges): 
        ys = [0]
        for i in range(1, self.k + 1):
            yi = (((challenges[i] % self.q) * pow(2, pow(2, i - 1, self.q), self.q) % self.q) + (self.As[i] % self.q)) % self.q 
            ys.append(yi)
        return ys

    # todo: might be redundant
    def compute_vprime(self):
        # return pow(self.h, pow(2, pow(2, self.k - self.l), int(totient(self.N))), self.N)
        print("v prime", pow(self.h, pow(2, pow(2, self.k + 1 - self.l)), self.N))
        a = pow(2, pow(2, self.k) - self.l, int(totient(self.N)))
        return pow(self.h, a, self.N)
         
# def main(): 
#     # TODO: does bits have to correspond to M length
#     committer = Committer(23123)
#     committer.compute_g()
#     committer.compute_u()
#     committer.compute_W()
#     print(committer.compute_rando_seq())
